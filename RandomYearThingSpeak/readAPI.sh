#!/usr/bin/env bash
#
# Este script se encarga de guardar en un archivo los datos de temperatura,
# viento y humedad de Cali en format JSON.
#
# El script espera un argumento que será el nombre del archivo se almacenaran
# los datos de temperatura, humedad y velocidad del viento
#
# Author: Pablo Andres Arroyo - 1525805
#         Gustavo Adolfo Aguirre - 1523452
#         Sebastian Serna - 1832018
# Date: 2019-09-17
#

leerSensor() { 
  echo "[readAPI] leyendo API"
  epoch=$( date +'%s')
  randomYear.sh > year_${epoch}.json
}

if [ ! $# -eq 1 ]; then
        echo "[readAPI] ${0} <filename>"
        exit -1
fi

yearTS_HOME="${HOME}/RandomYearThingSpeak"
PATH=${yearTS_HOME}:${PATH}
cd ${yearTS_HOME}
FILENAME=${1}

# Se valida si ya existe un archivo donde se haya guardado una lectura de
# temperatura anterior, weather_<EPOCH>.json
EXISTFILE=$(ls -l *.json | grep year_ | wc -l | awk '{print $1}')
if [ ${EXISTFILE} -eq 1 ]; then # Se encontro un archivo year_*.json
  echo "[readAPI] Se encontro archivo"
  EPOCH=$( ls *.json | grep year_ | cut -d '_' -f 2 | cut -d '.' -f 1)
  CURRENTEPOCH=$( date +'%s')
else
  echo "[readAPI] No se encontro archivo"
  leerSensor
  cat year_*.json > ${FILENAME}
  exit 0
fi

if [ $(( CURRENTEPOCH - EPOCH )) -lt 60 ]; then # El dato aun es vigente
  echo "[readAPI] Dato vigente"
  cat year_*.json > ${FILENAME}
  exit 0
fi
# Datos mayores a 5 minutos, desactualizados
echo "[readAPI] Dato desactualizado"
rm year_*.json
leerSensor
exit 0
